...
Zadatak se obavlja tako da se na temelju video sekvenci prometnica uz GPS lokacije pridružene video sekvencama određuje prisutnost sigurnosnog atributa na tom dijelu prometnice.
...
Drugi dio rada opisuje korištene metode i modele koji na temelju učenja na skupu podataka ostvaruju željenu funkciju koju možemo primjeniti na konkretan problem.
...
Pojedine video snimke je moguće preuzeti uz pripadajuće informacije o geo-lokaciji scena.
...
\caption{Prikaz ručno označene scene na kojoj je atribut pripajanja prisutan, preuzeto sa \citep{url:ftts_irap}}
...
\caption{Prikaz ručno označene scene na kojoj atribut pripajanja nije prisutan, preuzeto sa \citep{url:ftts_irap}}
...
\caption{Prikaz satelitske snimke s označenim segmentom atributa pripajanja, preuzeto sa \citep{url:google_maps}}
...
\caption{Prikaz video snimke s označenim segmentom atributa pripajanja, preuzeto sa \citep{url:ftts_irap}}
...
Konačan efekt jest da će se komponente parametara čije su parcijalne derivacije funkcije gubitka vrlo visokih vrijednosti korigirati sa značajno manjim stopama učenja, a komponente parametara čije su parcijalne derivacije funkcije gubitka relativno niskih vrijednosti, korigirat će se većim stopama učenja \citep{book:deeplearningbook} \citep{seminar:rela}.
...
\begin{equation}
 C^* = C + \lambda p(\mathbf{W}),
\end{equation}
gdje je $C^*$ nova funkcija gubitka koju optimiramo, $C$ je originalna funkcija gubitka, $p(\mathbf{W})$ predstavlja funkciju norme nad parametrima $\mathbf{W}$, a $\lambda$ je regularizacijski faktor.
...
Često korištene funkcije norme $p(\mathbf{W})$ jesu:
\begin{description}
 \item [L1 norma] $p(\mathbf{W})=\sum_{i=1}^n \left| w_i \right|$ --- L1 \engl{Lasso} regularizacija
 \item [L2 norma] $p(\mathbf{W})=\sqrt{\sum_{i=1}^n  w_i^2 }$ --- L2 \engl{Ridge} regularizacija
...
Vrijednost $x_0$ se fiksno postavlja na vrijednost $1$ iz razloga što želimo dozvoliti da model modelira pomak parametrima prvog stupca matrice $\mathbf{W}$.
...
\subsubsection{Aktivacijska funkcija identiteta}
Aktivacijska funkcija identiteta i njena derivacija definirane su kao: 
\begin{align}
 g^{(i)}(\mathbf{x})&=\mathbf{x}, \\
 \frac{dg^{(i)}(\mathbf{x})}{d\mathbf{x}}&=1
\end{align}
Aktivacijska funkcija identiteta propušta ulaz na izlaz bez promjene i pogodna je za korištenje u izlaznim slojevima regresijskih modela gdje nam je važan raspon vrijednosti. 
U praksi se ne koriste mreže kojima su svi slojevi s aktivacijskom funkcijom identiteta jer time i dalje imamo linearni model koji je manje ekspresivne moći od nelinearnog modela \citep{seminar:rela}.
...
\noindent Po dijelovima linearna aktivacijska funkcija predstavlja funkciju praga na vrijednosti $0$ ispod koje izlazne vrijednosti neurona nisu propuštene.
...
Nadalje, izračun vrijednosti mapa značajki se često generalizira kao i kod standardnog neurona pa se dodaje dodatan parametar kao vrijednost praga te se koriste aktivacijske funkcije.
...
Transformacija $k$-te komponente prethodno normaliziranog ulaznog vektora $\hat{x}$ nekog sloja definirana je na sljedeći način:
\begin{equation}
...
Normalizirane vrijednosti $\hat{\mathbf{x}}^{(k)}$ možemo promatrati kao ulaze u pod-sloj kojem je funkcija linearna, i to je upravo funkcija transformacije definirana u izrazu \ref{eq:bn_transformation}.
Na izlaz pod-sloja tada djeluje procesiranje originalnog sloja. 
Takvom arhitekturom ostvarili smo da svi pod-slojevi na ulaze dobivaju podatke s fiksnom distribucijom (s očekivanjem $0$ i varijancom $1$), što ubrzava učenje pod-slojeva, a time posljedično i samih slojeva i kompletne mreže \citep{article:bn_paper} \citep{seminar:rela}.
