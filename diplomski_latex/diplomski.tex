...
\includepdf[pages=-,width=\paperwidth]{izvornik.pdf}
...
Organizacija iRAP \engl{International Road Assessment Programe} ima za cilj upravo inspekciju kvalitete cesta pridruživanjem različitih sigurnosnih atributa segmentima prometnica.
...
Atribut koji sustav prepoznaje jest jedan od tipova križanja -- ``pripajanje trakova'' \engl{merge lane}. Kako je riječ o videosnimkama s engleskih cesta, pripajanja koja se prepoznaju bit će s lijeve strane.
...
Svi konvolucijski slojevi koriste po dijelovima linearnu aktivacijsku funkciju \engl{ReLU} te su regularizirani L2 normom parametara s faktorom regularizacije iznosa $5\cdot10^{-4}$.
...
\begin{algorithmic}
\WHILE{nije obrađen željen broj videosnimaka}
...
Purpose of training those models is to automate the assignment of safety attributes defined by the iRAP organization.
...
Dataset with discriminative labels is used to find the lowest resolution on which single images with discriminative pattern present are still successfully classified.
Determining low image resolution on which classification still works successfully is important because training on lower resolution is faster and it is possible to use larger mini batches.
...
Dataset with labels based on the data from FTTS iRAP system is used to measure performance gained by introducing the additional information through the image sequences.
It is shown that system can help with detection of wrongly labeled attributes.
